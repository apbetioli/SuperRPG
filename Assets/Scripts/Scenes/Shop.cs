...
public class Shop : MonoBehaviour {
	...
	void OnGUI(){
		...
		if (GameManager.GetInstance ().GetPlayer().weapon ==null || GameManager.GetInstance ().GetPlayer().weapon.GetType() == typeof(Fist)) {
			GUI.Label (new Rect (0, 50, 150, 20), "O que você quer comprar?");
			if (GUI.Button (new Rect (0, 70, 200, 50), "Espada de madeira"))
				GameManager.GetInstance ().GetPlayer().weapon = new WoodenSword();			
		}else
